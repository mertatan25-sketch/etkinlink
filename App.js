...
function seoGuncelle({ title, description, image } = {}) {
  ...
  const defaultTitle = 'etkinlink — Etkinliğin için en uygun mekanı bul';
  const defaultDesc = 'Düğün, nişan, kurumsal etkinlik ve daha fazlası için Türkiye\'nin en iyi mekanlarını keşfet. Ücretsiz fiyat teklifi al.';
...
function KullaniciGirisEkrani({ onGiris, onKapat, onGizlilikAc }) {
    ...
    return (
      <View style={styles.modalOverlay}>
        <Animated.View style={[styles.modal, { transform: [{ translateY: slideAnim }], maxHeight: '90%' }]}>
          ...
          <TouchableOpacity onPress={() => { setSifreSifirlamaModu(false); setSifreSifirlamaGonderildi(false); }}
          ...
          <Text style={[styles.modalBaslik, { textAlign: 'center' }]}>🔑 Şifremi Unuttum</Text>
          {sifreSifirlamaGonderildi ? (
            <View style={{ alignItems: 'center', paddingVertical: 20 }}>
              <Text style={{ fontSize: 40, marginBottom: 16 }}>📞</Text>
              <Text style={{ fontSize: 16, fontWeight: '700', color: C.success, marginBottom: 8 }}>Talebiniz Alındı!</Text>
              <Text style={{ color: C.textMid, textAlign: 'center', lineHeight: 22, fontSize: 14, marginBottom: 24 }}>
                Destek hattımız en kısa sürede sizi arayarak şifrenizi sıfırlamanıza yardımcı olacak.
              </Text>
              <Text style={{ color: C.gold, fontWeight: '700', fontSize: 14 }}>destek@etkinlink.com</Text>
              <TouchableOpacity
                style={[styles.teklifBtn, { backgroundColor: C.midnight, marginTop: 20, paddingHorizontal: 32 }]}
            ...
            <>
              <Text style={{ color: C.textMid, textAlign: 'center', marginBottom: 24, fontSize: 14, lineHeight: 20 }}>
                Kayıtlı telefon numaranızı girin. Destek ekibimiz şifrenizi sıfırlamanıza yardımcı olacak.
              </Text>
              ...
              <TouchableOpacity style={[styles.teklifBtn, { backgroundColor: C.midnight }]} onPress={sifreSifirlaGonder}>
                <Text style={styles.teklifBtnText}>📞 Sıfırlama Talebi Gönder</Text>
              </TouchableOpacity>
            </>
          )}
  ...
  return (
    <View style={styles.modalOverlay}>
      <Animated.View style={[styles.modal, { transform: [{ translateY: slideAnim }], maxHeight: '92%' }]}>
        <View style={[styles.modalBar, { backgroundColor: C.gold }]} />
...
// ── KULLANICI PANELİ ──────────────────────────────────────
function KullaniciPaneli({ kullanici, onCikis, onGizlilikAc, favorilerDis, karsilastirmaListesiDis }) {
  const [basvurular, setBasvurular] = useState([]);
  const [favoriler, setFavoriler] = useState(favorilerDis || []);
  const [favoriMekanlar, setFavoriMekanlar] = useState([]);
  const [karsilastirmaListesi, setKarsilastirmaListesi] = useState(karsilastirmaListesiDis || []);
  ...
  useEffect(() => {
    ...
    if (Platform.OS === 'web') {
      try {
        const f = localStorage.getItem(`etkinlink_favoriler_${kullanici.id}`);
        if (f) setFavoriler(JSON.parse(f));
        const k = localStorage.getItem(`etkinlink_karsilastirma_${kullanici.id}`);
        if (k) setKarsilastirmaListesi(JSON.parse(k));
      } catch(e) {}
    }
  }, []);
  ...
  function favoriKaldir(mekanId) {
    const yeni = favoriler.filter(id => id !== mekanId);
    setFavoriler(yeni);
    if (Platform.OS === 'web') {
      try { localStorage.setItem(`etkinlink_favoriler_${kullanici.id}`, JSON.stringify(yeni)); } catch(e) {}
    }
  }

  function karsilastirmadanKaldir(mekan) {
    const yeni = karsilastirmaListesi.filter(m => m.id !== mekan.id);
    setKarsilastirmaListesi(yeni);
    if (Platform.OS === 'web') {
      try { localStorage.setItem(`etkinlink_karsilastirma_${kullanici.id}`, JSON.stringify(yeni)); } catch(e) {}
    }
  }
  ...
  const KarsilastirmaTablosu = () => {
    ...
    const alanlar = [
      ...
      { key: 'valet', label: '🚗 Valet', fmt: v => v ? 'Var' : 'Yok' },
      { key: 'otopark', label: '🅿️ Otopark', fmt: v => v ? 'Var' : 'Yok' },
    ];
    return (
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: C.bg }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 52 }}>
        <View style={[styles.tabRow, { paddingHorizontal: 16 }]}>
          {[
            { id: 'basvurular', etiket: '📋 Başvurularım' },
        ...
        {aktifTab === 'karsilastirma' && (
          <View style={{ padding: 16 }}>
            ...
            {karsilastirmaListesi.length > 0 && (
              <TouchableOpacity
                onPress={() => { setKarsilastirmaListesi([]); if (Platform.OS === 'web') { try { localStorage.removeItem(`etkinlink_karsilastirma_${kullanici.id}`); } catch(e) {} } }}
...
function MekanDetay({ mekan, onGeri, onTeklif, kullanici, favoriler, onFavoriToggle }) {
  ...
  useEffect(() => {
    ...
    seoGuncelle({
      ...
      description: mekan.aciklama || `${mekan.isim}, ${mekan.sehir}. ${mekan.kapasite} kişiye kadar. etkinlink üzerinden ücretsiz fiyat teklifi alın.`,
  ...
  async function yorumGonder() {
    ...
    try {
      ...
      const tumYorumlar = [...yorumlar, { puan: yorumPuan }];
...
function KarsilastirmaModal({ mekanlar, onKapat, onTeklif, kullanici }) {
  const alanlar = [
    { key: 'kapasite', label: '👥 Kapasite', fmt: v => `${v} kişi` },
    { key: 'fiyat_aralik', label: '💰 Fiyat', fmt: v => v || '—' },
    { key: 'puan', label: '★ Puan', fmt: v => String(v) },
    { key: 'sehir', label: '📍 Şehir', fmt: v => v },
    { key: 'valet', label: '🚗 Valet', fmt: v => v ? 'Var' : 'Yok' },
    { key: 'otopark', label: '🅿️ Otopark', fmt: v => v ? 'Var' : 'Yok' },
  ];
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ paddingHorizontal: 20 }}>
          ...
          {alanlar.map((alan, ai) => (
            <View key={ai} style={{ flexDirection: 'row', gap: 8, paddingVertical: 10, borderTopWidth: 1, borderTopColor: C.border, alignItems: 'center' }}>
              ...
              {mekanlar.map((m, mi) => {
                const vals = mekanlar.map(x => alan.key === 'kapasite' ? x[alan.key] : x[alan.key]);
                const isNum = alan.key === 'kapasite' || alan.key === 'puan';
  ...
  return (
    <Animated.View style={[
      ...
      <TouchableOpacity onPress={() => onDetay(mekan)} activeOpacity={0.95}>
        ...
        <TouchableOpacity
          onPress={() => onFavoriToggle && onFavoriToggle(mekan.id)}
          style={{ position: 'absolute', top: 14, right: mekan.one_cikan ? 70 : 58, width: 34, height: 34, borderRadius: 17,
...
export default function App() {
  ...
  const [favoriler, setFavoriler] = useState(() => {
    if (Platform.OS === 'web') {
      try { const f = localStorage.getItem('etkinlink_favoriler'); return f ? JSON.parse(f) : []; } catch(e) {}
    }
    return [];
  });
  ...
  const [karsilastirmaListesi, setKarsilastirmaListesi] = useState(() => {
    if (Platform.OS === 'web') {
      try { const k = localStorage.getItem('etkinlink_karsilastirma'); return k ? JSON.parse(k) : []; } catch(e) {}
    }
    return [];
  });
  const [karsilastirmaAcik, setKarsilastirmaAcik] = useState(false);
  ...
  function favoriToggle(mekanId) {
    setFavoriler(prev => {
      ...
      if (Platform.OS === 'web') { try { localStorage.setItem('etkinlink_favoriler', JSON.stringify(yeni)); } catch(e) {} }
      return yeni;
    });
  }
  ...
  function karsilastirmaToggle(mekan) {
    setKarsilastirmaListesi(prev => {
      const var_ = prev.some(m => m.id === mekan.id);
      const yeni = var_ ? prev.filter(m => m.id !== mekan.id) : prev.length >= 3 ? [...prev.slice(1), mekan] : [...prev, mekan];
      if (Platform.OS === 'web') { try { localStorage.setItem('etkinlink_karsilastirma', JSON.stringify(yeni)); } catch(e) {} }
      return yeni;
    });
  }
  ...
  if (ekran === 'kullaniciPanel' && kullanici) return <KullaniciPaneli kullanici={kullanici} onCikis={kullaniciCikis} onGizlilikAc={() => setGizlilikAcik(true)} favorilerDis={favoriler} karsilastirmaListesiDis={karsilastirmaListesi} />;
